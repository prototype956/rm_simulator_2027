...
第一阶段火控机制按模块逐步开发。当前模块 1 已加入机器人身份、规则预设与独立状态，
尚未接入热量、弹量或扣血结算；配置与规则来源见 [火控基础机制说明](docs/COMBAT_PHASE1.md)。

当前协议版本为不兼容旧版的 v6。图像池大小跟随 `config.toml` 的 `capture.color` 动态创建，
Daedalus 默认发布 BGR8。图像索引、采集时间、内参、空间变换、底盘观测、弹丸累计统计和
本帧真值放在同一个 `CapturedFrameMeta` 三缓冲槽中，像素复制完成后一次提交。v6 将 v5 的
32 字节保留区定义为 17 mm 发射、装甲命中、能量机关命中和飞镖发射计数，总 ABI 尺寸及
关键偏移保持不变；v5 客户端会因 magic/version 不匹配而明确拒绝连接。
...
弹丸解释为未命中。Talos 路径的每个 `fire_advice` 上升沿生成一发弹丸，`0.05 s` 最大射频由
视觉火控保证；手动 `Space` 路径仍使用仿真器自己的冷却计时器。
