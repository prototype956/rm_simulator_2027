...
mod tests {
    ...
    #[test]
    fn capture_projection_maps_to_resized_window_and_ui_scale() {
        assert_eq!(
            screen_anchor(
        ...
        assert_eq!(
            ...
            Some(Vec2::new(480.0, 90.0))
        );
        for point in [
            Vec3::new(1.1, 0.0, 0.5),
            Vec3::new(0.0, -1.1, 0.5),
        ...
        assert!(screen_anchor(Vec3::splat(0.5), Rect::default(), 1.0).is_none());
        let letterbox = preview_rect(Vec2::new(800.0, 600.0), Vec2::new(1280.0, 720.0)).unwrap();
        assert_eq!(letterbox, Rect::new(0.0, 75.0, 800.0, 525.0));
        assert_eq!(
            screen_anchor(Vec3::new(0.0, 1.0, 0.5), letterbox, 1.0),
            Some(Vec2::new(400.0, 75.0))
        );
        let pillarbox = preview_rect(Vec2::new(1280.0, 720.0), Vec2::new(800.0, 600.0)).unwrap();
        assert!(pillarbox.min.abs_diff_eq(Vec2::new(160.0, 0.0), 0.001));
        assert!(pillarbox.max.abs_diff_eq(Vec2::new(1120.0, 720.0), 0.001));
    }

    #[test]
    fn mesh_top_accounts_for_rotation_and_scale() {
        ...
        assert!((top_of_mesh(&bounds, &transform) - 6.0).abs() < 1e-5);
    }
}
