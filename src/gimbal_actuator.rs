...
/// Explicit clock used by lockstep callers. No host time is read while advancing it.
#[derive(Resource)]
...
impl Default for ActuatorClock {
    fn default() -> Self {
        Self {
            origin: Instant::now(),
            elapsed: Duration::ZERO,
            epoch_ns: 1_000_000_000_000_000_000,
        }
    }
}
impl ActuatorClock {
    ...
    pub fn timestamp_ns(&self) -> u64 {
        self.epoch_ns + self.elapsed.as_nanos() as u64
    }
}
...
impl GimbalCommandInbox {
    ...
    /// Submission confirms transport admission, not an actuator consumption or a shot.
    pub fn submit(&self, command: GimbalCmd, clock: &ActuatorClock) -> Result<(), &'static str> {
        let mut q = self.queue.lock().map_err(|_| "inbox poisoned")?;
        if q.len() >= COMMAND_QUEUE_CAPACITY {
            return Err("command inbox full");
        }
        q.push_back(ReceivedCommand {
            command,
            received_at: clock.instant(),
            received_system_ns: clock.timestamp_ns(),
        });
        Ok(())
    }

    #[cfg(feature = "training")]
    pub(crate) fn pending_for_reset(&self) -> Result<Vec<serde_json::Value>, &'static str> {
        let queue = self.queue.lock().map_err(|_| "inbox poisoned")?;
        Ok(queue
            .iter()
            .map(|r| reset_command_data(&r.command, "inbox", None))
            .collect())
    }

    #[cfg(feature = "training")]
    pub(crate) fn suppress_fire_for_window(&self) -> Result<(), &'static str> {
        for received in self.queue.lock().map_err(|_| "inbox poisoned")?.iter_mut() {
            received.command.fire_advice = 0;
        }
        Ok(())
    }
...
#[cfg(feature = "training")]
fn reset_command_data(
    command: &GimbalCmd,
    stage: &str,
    activation_ns: Option<u64>,
) -> serde_json::Value {
    serde_json::json!({"command_timestamp_ns":command.timestamp_ns,
        "source_round_id":command.source_round_id,"source_frame_sequence":command.source_frame_sequence,
        "fire_level":command.fire_advice,"stage":stage,"activate_timestamp_ns":activation_ns,
        "status":"cancelled_by_reset"})
}

#[cfg(feature = "training")]
impl GimbalActuator {
    pub(crate) fn suppress_fire_for_window(&mut self) {
        for delayed in &mut self.pending {
            delayed.command.fire_advice = 0;
        }
        self.fire_level = false;
    }

    pub(crate) fn pending_for_reset(&self) -> Vec<serde_json::Value> {
        self.pending
            .iter()
            .map(|p| reset_command_data(&p.command, "command_delay", Some(p.activate_system_ns)))
            .collect()
    }
}
...
pub fn pose_angles(rotation: Quat, yaw_reference: f64) -> (f64, f64) {
...
pub fn muzzle_world_rotation(yaw: f64, pitch: f64) -> Quat {
    Quat::from_euler(
        EulerRot::YXZ,
...
pub fn solve_gimbal_local_rotation(
...
fn command_target(command: GimbalCmd, pitch_limit: f64, activated_at: Instant) -> ActiveCommand {
    let valid =
...
impl GimbalActuator {
    /// Set an enabled initial state before the first submitted training command.
    pub fn initialize(&mut self, yaw: f64, pitch: f64, clock: &ActuatorClock) {
        self.reset_from_pose(yaw, pitch, clock.instant());
        self.was_enabled = true;
        self.accept_after = clock.instant() - Duration::from_nanos(1);
        self.min_command_timestamp_ns = 0;
    }

    pub fn reset_from_pose(&mut self, yaw: f64, pitch: f64, now: Instant) {
...
pub fn update_gimbal_actuator(
    ...
    explicit_clock: Option<Res<ActuatorClock>>,
    ...
    let now = explicit_clock
        .as_ref()
        .map_or_else(Instant::now, |c| c.instant());
    let timestamp_ns = explicit_clock
        .as_ref()
        .map_or_else(system_now_ns, |c| c.timestamp_ns());
    let alive = robots
    ...
    if !actuator.was_enabled || !actuator.initialized {
        ...
        actuator.min_command_timestamp_ns = timestamp_ns;
        inbox.clear();
    }
    ...
    if explicit_clock.is_none() && now.saturating_duration_since(cursor) > MAX_CATCH_UP {
        ...
        .is_some_and(|event| event.activate_at <= now)
    {
        ...
        let mut active = command_target(event.command, pitch_limit, activate_at);
